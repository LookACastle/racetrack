...
from lifecycle.job.registry import list_jobs_registry
...
def build_job_dependencies_graph(config: Config, auth_subject: Optional[models.AuthSubject]) -> JobGraph:
    jobs: List[JobDto] = list_jobs_registry(config, auth_subject)
    ...
    for family_name in family_names:
        jobs_details = _list_family_jobs_details(jobs, family_name, config)
        nodes.append(JobGraphNode(
...
def _list_family_jobs_details(jobs: List[JobDto], family_name: str, config: Config) -> str:
