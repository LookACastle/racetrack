...
@db_access
def list_jobs_models() -> Iterable[models.Job]:
    """List deployed job stored in registry database"""
...
def read_latest_job_model(job_name: str) -> models.Job:
    jobs_queryset = models.Job.objects.filter(name=job_name)
    if jobs_queryset.count() == 0:
        raise EntityNotFound(f'No job named {job_name}')
    jobs: List[models.Job] = list(jobs_queryset)
