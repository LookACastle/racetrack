...
def list_jobs_registry(config: Config, auth_subject: Optional[models.AuthSubject] = None) -> List[JobDto]:
    """List jobs getting results from registry (Database)"""
    if auth_subject is None:
        return [job_model_to_dto(job, config) for job in models_registry.list_jobs_models()]
    else:
        jobs = [job_model_to_dto(job, config) for job in models_registry.list_jobs_models()]
...
def read_job_family(job_family: str) -> JobFamilyDto:
    """Read jobs family from registry (Database)"""
...
def sync_registry_jobs(config: Config, plugin_engine: PluginEngine):
    ...
    with wrap_context('synchronizing job'):
        cluster_jobs = _generate_jobs_map(list_cluster_jobs(config, plugin_engine))
        registry_jobs = _generate_jobs_map(list_jobs_registry(config))
        logger.debug(f'Found {len(cluster_jobs)} jobs in the cluster, {len(registry_jobs)} in the database')
        jobs_status_count: Dict[str, int] = defaultdict(int)
        ...
        for job_id, registry_job in registry_jobs.items():
            ...
            jobs_status_count[registry_job.status] += 1
        ...
        for job_id, cluster_job in cluster_jobs.items():
            if job_id not in registry_jobs:
                ...
                jobs_status_count[cluster_job.status] += 1

        logger.debug(f'Jobs synchronized, count by status: {dict(jobs_status_count)}')
...
def _generate_jobs_map(jobs: Iterable[JobDto]) -> Dict[str, JobDto]:
