...
- Racetrack instance can operate with multiple infrastructure targets.
  ...
  The infrastructure target is displayed next to the job name, on Dashboard's list.
...
- Job can be composed of multiple Docker images provided by a plugin or user's manifest.
...
- Exact job type version can be checked at Job's `/health` endpoint.
...
- Job webviews can now serve ASGI applications (like FastAPI)
...
- One job type can be installed in multiple versions at the same time.
  Users have to pick one of these versions and specify it in the manifest of their job,
...
- Base Job images are built inside Racetrack by image-builder
  so it's no longer needed to push images prior to the plugin release.

## [2.3.0] - 2022-09-23
### Added
- Job access can be narrowed down to single endpoints.
  When adding Auth Resource Permission (in Admin panel), there is a new `endpoint` field,
  which narrows down the permission only to this particular Job's endpoint.
  If not set, it covers all endpoints (just as other resource filters do).
  For instance, ESC can have a permission with `call_job` scope to only call one endpoint.
- OpenTelemetry can be turned on in Racetrack.
  Given the OTLP collector endpoint,
  the traces from lifecycle, pub and job will be sent there.
...
## [2.2.1] - 2022-08-25
### Changed
- Maximum amount of memory for a job is reduced to `1GiB` by default
...
- Job versions in PUB's metrics are always resolved to the exact version
...
### Changed
- Function `job_job_types` of plugins changed its signature.
...
- A namespace for job workloads running on Kubernetes is now configurable
...
- Overall requests performance has been improved by switching from 
  ...
  Job server should now be less laggy under heavy load.
  Additionaly, FastAPI comes with better request validation and newer SwaggerUI page.
  Job redeployment is needed for the changes to take effect.

## [2.1.1] - 2022-07-25
### Added
- Plugins can provide its own Job Deployers
...
### Changed
- Job's metrics now contain number of requests and duration for each endpoint
  (including auxiliary endpoints).

### Fixed
- Fixed redeploying jobs from the dashboard
- Making multiple attempts to deploy the same job at the same time is now disallowed.
- "In progress" deployments are halted when Lifecycle restarts
  (due to upgrade or eviction)

## [2.1.0] - 2022-07-08
### Changed
- Job secrets (git credentials and secret vars) are kept in Racetrack,
  so jobs are fully reproducible again.
  In other words, it is technically possible to rebuild and reprovision job
  by anyone who is allowed to do so (anyone with permissions, not just the author)
  to apply latest Racetrack changes.
  You can click "Rebuild and provision" to do a complete redeployment of a job,
...
- Racetrack client has new command `racetrack run-local . <racetrack_url>`
  allowing to run job locally based on docker image built by remote Racetrack.
  Docker has to be installed on your system.

### Changed
-   Job permissions model is more restrictive.
    Now users, ESCs and job families can be granted fine-grained permissions.
    Users can have access to a subset of jobs and they
    can see and manage only those that are allowed for them.
    Old permissions have been migrated (Job-to-Job and ESC-to-job access).

    By default, Users can now do the following:
    
    - read all job status (browse on a dashboard)
    - call endpoints of every job
    - deploy and redeploy jobs
    - delete only jobs that has been deployed by the user (starting from the newly deployed ones)

    Admin can grant or revoke permissions.
    Permisssion can cover either all jobs, whole job family or a single job.
...
- A method for calling job from inside of other job has been changed due to new auth model.
  See [python-chain sample](../sample/python-chain/entrypoint.py).
  All jobs calling another job are required to be redeployed (to use new tokens).

- Since there is one type of tokens, 
  Auth header has been unified and renamed to `X-Racetrack-Auth`.
  It should be used no matter if it's ESC, User or a Job.
...
- Racetrack services has been adjusted to the new URL format. 
  ...
  Thus, Job instances can be accessed at: `https://ikp-rt.<cluster.address>/pub/job/<name>/<version>/<path>`.

## [1.2.0] - 2022-05-31
### Added
- `job.yaml` manifest file supports overlays.
...
- Critical actions like deleting job or redeploying it
...
### Changed
- Once a job name (and version) has been claimed,
...
## [1.0.2] - 2022-04-06
### Changed
- Set default job maximum memory to 8GiB

## [1.0.1] - 2022-04-06
### Changed
- Job dependency graph is interactive, nodes are movable.
  Clicking on a node filters out all its neighbours 
  to see which jobs can be accessed by particular ESC 
  or which ESCs have access to a particular job.

### Changed
- Set default job minimum memory to 150MiB

## [1.0.0] - 2022-03-29
### Added
- Job manifest can declare resources to be allocated to the Job
  with `resources` field, including minimum/maximum memory, minimum/maximum CPU cores.
  See [Job Manifest File](user.md#the-job-manifest-file)
- Dashboard shows datetime when the last call was made to a job.
  Job redeployment is needed for the changes to take effect.
- Dashboard has a new "Portfolio" tab for browsing jobs freely
...
- Job returns 400 BAD REQUEST response in case of missing argument,
  unexpected argument or any other TypeError.

### Fixed
- Fixed Jobs getting Lost.
  Jobs shouldn't wrongly change its status to "Lost" any longer.
...
## [0.7.4] - 2022-03-01
### Added
- PUB collects histogram of job response times
  and metrics related to endless requests and response codes.

## [0.7.3] - 2022-02-28
### Changed
- `GET /metrics 200` requests are hidden from access logs.
- Reading from `stdin` is prohibited at job initialization and it raises an error once detected,
  showing stack trace with the place where there was an attempt.
  Job doesn't go unresponsive any longer, awaiting input indefinitely.

### Fixed
- Job logs doesn't show misleading "Timing out client" errors anymore.
  It was related to HTTP sessions kept alive by browsers, not actual job request time-outs.
...
### Changed
- Job manifest is checked against using HTTPS git remote only (SSH is not supported).
...
- Job access logs have shorter, concise format.
  Time, HTTP method, path and response code are the only ones displayed.

## [0.7.0] - 2022-01-31
### Added
- When accessing a Job through a PUB, there can be used wildcard version format: `1.x` or `1.2.x`.
  This points to a highest stable version of the Job that matches the pattern.
  For instance, `1.x` keeps track of all v1 updates, but skips next major versions `2.0.0` and above.
  Job redeployment is needed for the changes to take effect.

### Changed
- "latest" job version now points to the highest version disregarding versions with any label (eg. `-dev`).
...
- Phases of deployment are tracked by showing "building image", "creating cluster resources",
  "initializing Job entrypoint", etc. in RT client in place of bare "deployment in progress..." message.

### Changed
- Job versions are validated according to [SemVer format](https://semver.org/).
  Version should match `MAJOR.MINOR.PATCH[LABEL]`, eg.: `0.1.2` or `0.0.0-dev`.

### Fixed
- Zero-value metrics are shown properly at `/metrics` endpoint of a Job.
...
- Metrics from Postgres database and PUB are exported. 
  It gives better monitoring of Job requests performance.
...
## [0.5.0] - 2022-01-10
### Changed
- Overwriting jobs is disabled.
  It's not allowed to deploy a job with the same name & version as an existing one,
...
## [0.4.0] - 2022-01-03
### Added
- Number of running instances of the Job 
...
- Deployed jobs can be deleted through racetrack client using new command:
  `racetrack delete <workdir> <racetrack_url> [--version X.Y.Z]`.

## [0.3.5] - 2021-12-09
### Added
- Job endpoints that are supposed to be accessed publicly (without authentication)
...
### Fixed
- Improved overall Job response time.

## [0.3.4] - 2021-12-06
### Changed
- Job's `/live`, `/ready` and `/health` endpoints doesn't require authentication.

## [0.3.3] - 2021-11-22
### Changed
- Job "dashboard" renamed to "webview". See Python Job Type docs: Custom Webview UI.
...
## [0.3.1] - 2021-11-15
### Added
- New optional field in Manifest: `labels` - dictionary with metadata describing job for humans.
  Labels are shown on a dashboard page besides job name.
...
- Error returned by Job contains more information (including Lifecycle response)
  and uses correct status code (instead of 500)
- Fixed showing spurious success of Job redeployment 
  (ensure new Job responds to health probes).

## [0.3.0] - 2021-11-08
### Added
- `racetrack logs` command follows Job logs when called along with `--follow` flag.
### Changed
- Lifecycle API is fully authenticated, every request to Job needs to have either User, ESC or Job token.
- `X-Racetrack-Caller` header has changed to `X-Racetrack-Esc-Auth`

## [0.2.1] - 2021-10-25
### Added
- Multiple versions of the same job are allowed. 
  Obsolete versions are not removed automatically. 
  ESC permissions are assigned to whole jobs family by its name.
...
- Job logs are displayed on Dashboard under option "Logs" / "Runtime logs". 

### Changed
- Job base URL is changed to `/ikp-rt/pub/job/<job-name>/<version>/`. 
...
- Swagger UI allows to set `X-Racetrack-Caller` header for making authorized Job calls.
...
### Changed
- Dashboard allows users to register, to be able to login and view list of Jobs.
...
## [0.1.0] - 2021-09-27
### Changed
- Hide "Open" button for erroneous Job on Dashboard page.
...
- Racetrack client infers full lifecycle URL, namely https protocol and `/ikp-rt/lifecycle` path, if not given. 
  For instance, `ikp-dev-cluster.example.com` is a valid URL to deploy jobs.
- Admin panel not available on Dashboard (moved to Lifecycle).

### Fixed
- Use DNS hostname of a cluster instead of raw IP address when redirecting to a Job URL
  (fixed in dashboard "Open" link as well as in `racetrack deploy` output).

## [0.0.17] - 2021-09-20
### Added
- Job serves 3 version numbers at `/health` endpoint: 
    - `git_version` - arising from the git history of a job source code,
    - `job_version` - taken from `version` field in manifest (this is also displayed at SwaggerUI page).
    - `deployed_by_racetrack_version` - version of the Racetrack the Job was deployed with.
...
- Authorization to PUB (optional right now) for accessing Job `/perform`.
...
- Dashboard page shows both dates for each Job: "Created at" and "Last updated" 
  (deployed at) along with its age (e.g. "5 minutes ago", "5 days ago").

### Changed
- In case of python3 initialization error, full traceback coming from job is 
...
- Jobs list on Dashboard is ordered by last deployment date.

### Fixed
- Fix retrieving logs right after deploying a Job.
- Increased timeout for creating a pod. It takes into account longer image pulling.

## [0.0.15] - 2021-09-07
### Added
- New required `owner_email` field (email address of the Job's owner to reach out) in manifest
- List of Jobs (in Dashboard) shows who deployed the job recently 
...
## [0.0.14] - 2021-09-06
### Changed
- Job has read-write access to local working directory.
